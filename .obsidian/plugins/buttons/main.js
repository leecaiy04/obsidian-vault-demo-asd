...
var require_postfix_evaluator = __commonJS({
  "node_modules/math-expression-evaluator/src/postfix_evaluator.js"(exports, module2) {
    ...
    Mexp.prototype.postfixEval = function(UserDefined) {
      ...
      if (stack.length > 1) {
        throw new Mexp.Exception("未捕获的语法错误");
      }
...
async function templater(app, template2, _target) {
  ...
  if (!exists) {
    new import_obsidian2.Notice("未安装Templater。请安装它。");
    return;
  }
  try {
...
var nameModal = class extends import_obsidian7.Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("h1", { text: "新笔记名称" });
    new import_obsidian7.Setting(contentEl).setName("名称").addText((text2) => {
      text2.setPlaceholder(this.defaultName);
      text2.onChange((value) => {
        this.result = value;
      });
    });
    new import_obsidian7.Setting(contentEl).addButton((btn) => btn.setButtonText("创建笔记").setCta().onClick(() => {
      this.close();
...
var createNote = async (app, type, folder, prompt, filePath, isTemplater) => {
  ...
  if (path) {
    ...
    if (directoryPath && !app.vault.getAbstractFileByPath(directoryPath)) {
      console.log("尝试在以下位置创建文件夹：", directoryPath);
...
var INVALID_MODIFIER_ERROR = 'Popper: 修改器 "%s" 提供了一个无效的 %s 属性,期望 %s 但得到 %s';
var MISSING_DEPENDENCY_ERROR = 'Popper: 修改器 "%s" 需要 "%s",但 "%s" 修改器不可用';
